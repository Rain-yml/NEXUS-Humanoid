...
# --local-ranks-filter ${LOG_RANK} --role rank --tee 3 --log-dir /var/log/torch/elastic \
# -m torchtitan.experiments.vem.train_vdit --job.config_file ./torchtitan/experiments/vem/configs/${CONFIG_FILE} \
...
-m torchtitan.experiments.vem.train_sm_gen --job.config_file ./torchtitan/experiments/vem/configs/${CONFIG_FILE} \
--job.dump_folder=${LOG_PATH} \
--checkpoint.path_override=${CHECKPOINT_PATH} \
--checkpoint.load_path_override=${PRETRAIN_PATH} \
$overrides 