...
class MetricsProcessor:
    ...
    def log(
        ...
        # assert (
        #     self.num_flops_per_token > 0 or self.num_flops_since_last_log > 0
        # ), "num_flops_per_token or num_flops_since_last_log must be set"
        ...
        if self.num_flops_per_token > 0:
            ...
            mfu = 100 * self.num_flops_since_last_log / (time_delta * self.parallel_dims.non_data_parallel_size) / self.gpu_peak_flops
            tflops = self.num_flops_since_last_log / (time_delta * self.parallel_dims.non_data_parallel_size) / 1e12                      

        self.accumulated_tokens += self.ntokens_since_last_log
        ...
        color = self.color
        extra_keys = ['acc', 'edge', 'face', 'prec', 'rec', 'fs', 'f_prec', 'f_rec', 'f_fs', 'ori_acc']
        extra_log = "  ".join([f"{key}: {metrics[key]:.4f}" for key in extra_keys if key in metrics])
        # if "acc" in metrics:
        logger.info(
            ...
            f" {color.white}{extra_log}{color.reset}"
        )
        # else:
        #     logger.info(
        #         f"{color.red}step: {step:2}  "
    ...
    def log_validation(self, step: int, loss: float, extra_metrics = None):
        time_delta = time.perf_counter() - self.time_last_log
        ...
        color = self.color
        logger.info(
            f"{color.red}validate step: {step:2}  "
