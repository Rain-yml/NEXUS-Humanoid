...
class ModelWrapper(Stateful):
    def __init__(self, model: nn.Module | list[nn.Module]) -> None:
        ...
        self._state_dict_exclude_keys: list[str] = []
    ...
    def state_dict(self) -> dict[str, Any]:
        ...
        if self._state_dict_exclude_keys:
            ignore_keys = [k for k in state_dict if any(ek in k for ek in self._state_dict_exclude_keys)]
            logger.info(f"ignore_keys: {', '.join(ignore_keys)}")
            state_dict = {
                k: v
                for k, v in state_dict.items()
                if not any(ek in k for ek in self._state_dict_exclude_keys)
            }
        return state_dict
...
@contextmanager
def set_model_states_exclude_keys(model: ModelWrapper, exclude_keys: list[str]):
    old_exclude_keys = model._state_dict_exclude_keys
    model._state_dict_exclude_keys = exclude_keys
    yield
    model._state_dict_exclude_keys = old_exclude_keys
...
class CheckpointManager:
    ...
    def __init__(
        ...
        if ckpt_config.path_override is not None:
            self.folder = ckpt_config.path_override
        
        if ckpt_config.load_path_override is not None:
            self.load_folder = ckpt_config.load_path_override
        else:
            self.load_folder = self.folder
        self.load_model_exclude_keys = ckpt_config.load_model_exclude_keys
        ...
        self.persistent_steps = ckpt_config.persistent_steps
    ...
    def load(self, step: int = -1) -> bool:
        ...
        if not self.enable_checkpoint or not os.path.isdir(self.load_folder):
            return False

        if step == -1:
            step = self._find_load_step(self.load_folder)
            if step == -1:
                return False
        ...
        logger.info(f"\033[32mLoading the checkpoint at step {step}.\033[39m")
        begin = time.monotonic()
        states = self._states_to_load(step)

        with set_model_states_exclude_keys(states[MODEL], self.load_model_exclude_keys):
    ...
    def _purge_stale_checkpoints(self):
        if (
            ...
            for step, path in to_delete:
                if step in self.persistent_steps:
                    continue
