"""Train semantic joints in one NEXUS stream conditioned on clean mesh tokens."""
# Owned copy of experiments/humanoid/trainer.py.
...
class DiffusionTrainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def train_step(self, input_dict: JointOctreeBatch):
        ...
        if joint_mask is None or not joint_mask.any() or joint_mask.all():
            raise ValueError(
                "A single-stream batch must contain both mesh and joint tokens"
            )

        # Noise is sampled in the original latent space but applied only to joints.
        ...
        # Mesh occupancy is a clean spatial condition at t=0. Joint occupancy is
        # the only diffusion state and follows the original NEXUS flow path.
        joint_mask_expanded = joint_mask.unsqueeze(-1)
        diffused_latents = (1.0 - sigmas) * latents + sigmas * noise
        noisy_latents = torch.where(joint_mask_expanded, diffused_latents, latents)
        model_timesteps = torch.where(
            joint_mask, timesteps, torch.zeros_like(timesteps)
        )
        ...
        joint_noisy_latents = noisy_latents[joint_mask]
        joint_latents = latents[joint_mask]
        joint_noise = noise[joint_mask]
        joint_sigmas = sigmas[joint_mask]
        joint_prediction = model_pred[joint_mask]
        weighting = torch.ones_like(joint_sigmas)

        # Preserve the NEXUS objective exactly, restricted to joint tokens.
        if self.job_config.training.loss_type == "xpred-vloss":
            v_pred = (joint_noisy_latents - joint_prediction) / torch.clamp(
                joint_sigmas, min=0.05
            )
            target = (joint_noisy_latents - joint_latents) / torch.clamp(
                joint_sigmas, min=0.05
            )
        elif self.job_config.training.loss_type == "vpred-vloss":
            v_pred = joint_prediction
            target = (joint_noise - joint_latents).detach()
        else:
