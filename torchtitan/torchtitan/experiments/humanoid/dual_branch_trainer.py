"""Joint-only training with a frozen NEXUS mesh branch as 3D context."""
...
import torchtitan.experiments.humanoid.train_spec  # noqa: F401 - registers the train spec
from torchtitan.experiments.humanoid.arguments import Scheduler
...
from torchtitan.experiments.humanoid.data.split_octree_streams import (
    split_octree_streams,
)
...
class DiffusionTrainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        if job_config.training.pretrained_path:
            model_args = replace(model_args, pretrained_path=job_config.training.pretrained_path)
    ...
    def train_step(self, input_dict: JointOctreeBatch):
        ...
        mesh, joints = split_octree_streams(input_dict)
        conditions = self.prepare_conditions(input_dict, self.job_config)
        sequence_sigmas = compute_density_for_timestep_sampling(
            weighting_scheme=self.scheduler_config.t_sampling_scheme,
            batch_size=mesh.cu_seqlens.numel() - 1,
            ...
            dtype=mesh.occupancy.dtype,
            device=mesh.occupancy.device,
        )
        mesh_sigmas = torch.repeat_interleave(
            sequence_sigmas, torch.diff(mesh.cu_seqlens).to(dtype=torch.long)
        ).unsqueeze(-1)
        joint_sigmas = torch.repeat_interleave(
            sequence_sigmas, torch.diff(joints.cu_seqlens).to(dtype=torch.long)
        ).unsqueeze(-1)
        mesh_noise = torch.randn_like(mesh.occupancy) * self.scheduler_config.noise_std
        joint_noise = torch.randn_like(joints.occupancy) * self.scheduler_config.noise_std
        mesh_noisy = (1.0 - mesh_sigmas) * mesh.occupancy + mesh_sigmas * mesh_noise
        joint_noisy = (1.0 - joint_sigmas) * joints.occupancy + joint_sigmas * joint_noise

        mesh_prediction, joint_prediction = model(
            mesh_x_t=mesh_noisy,
            joint_x_t=joint_noisy,
            mesh_t=mesh_sigmas.squeeze(-1) * self.scheduler_config.num_train_timesteps,
            joint_t=joint_sigmas.squeeze(-1) * self.scheduler_config.num_train_timesteps,
            mesh_centers=mesh.centers,
            joint_centers=joints.centers,
            mesh_depths=mesh.depths,
            joint_depths=joints.depths,
            mesh_cu_seqlens=mesh.cu_seqlens,
            joint_cu_seqlens=joints.cu_seqlens,
            joint_ids=joints.joint_ids,
        ...
        self.metrics_processor.ntokens_since_last_log += (
            mesh.occupancy.shape[0] + joints.occupancy.shape[0]
        )
        condition_lens = [1] * (mesh.cu_seqlens.numel() - 1)
        self.metrics_processor.num_flops_since_last_log += unwrapped_model.flops(
            mesh.cu_seqlens, condition_lens
        )

        if self.job_config.training.loss_type == "xpred-vloss":
            joint_v_pred = (joint_noisy - joint_prediction) / torch.clamp(
                joint_sigmas, min=0.05
            )
            joint_target = (joint_noisy - joints.occupancy) / torch.clamp(
                joint_sigmas, min=0.05
            )
            mesh_v_pred = (mesh_noisy - mesh_prediction) / torch.clamp(
                mesh_sigmas, min=0.05
            )
            mesh_target = (mesh_noisy - mesh.occupancy) / torch.clamp(
                mesh_sigmas, min=0.05
            )
        elif self.job_config.training.loss_type == "vpred-vloss":
            joint_v_pred = joint_prediction
            joint_target = (joint_noise - joints.occupancy).detach()
            mesh_v_pred = mesh_prediction
            mesh_target = (mesh_noise - mesh.occupancy).detach()
        else:
            raise NotImplementedError(
        ...
        mesh_gradients = [
            name
            for name, parameter in unwrapped_model.mesh_branch.named_parameters()
            if parameter.grad is not None
        ]
        if mesh_gradients:
            raise RuntimeError(
                f"Frozen mesh branch received gradients: {mesh_gradients[:5]}"
            )
        attend_grad_sq = torch.zeros((), device=self.device, dtype=torch.float32)
        for parameter in unwrapped_model.joint_attend_mesh.parameters():
            if parameter.grad is None:
                continue
            gradient = parameter.grad.detach()
            if hasattr(gradient, "to_local"):
                gradient = gradient.to_local()
            attend_grad_sq += gradient.float().square().sum()
        attend_grad_norm = attend_grad_sq.sqrt()
        ...
        if (
            ...
            global_avg_loss, global_max_loss = (
                dist_utils.dist_mean(joint_loss.detach(), world_mesh["dp_cp"], ft_pg),
                dist_utils.dist_max(joint_loss.detach(), world_mesh["dp_cp"], ft_pg),
            )
        else:
            global_avg_loss = global_max_loss = joint_loss.detach().item()
        self.metrics_processor.log(
            ...
            extra_metrics={
                ...
                "lr": lr,
                "mesh_loss": mesh_loss.detach().item(),
                "joint_loss": joint_loss.detach().item(),
                "joint_mse": joint_mse.detach().item(),
                "joint_sequences": joint_sequence_count,
                "joint_attend_mesh_grad_norm": attend_grad_norm.item(),
            },
        )
