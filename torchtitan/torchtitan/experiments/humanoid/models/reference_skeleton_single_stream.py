...
class ReferenceSkeletonSingleStreamDiffusionModel(ModelMixin, ConfigMixin):
    """
    八叉树逐层扩散模型
    ...
    def __init__(
        ...
        reference_encoder_edge_dim: int = 128,
        reference_encoder_num_blocks: int = 3,
    ):
        super().__init__()
        ...
        self.reference_skeleton_encoder = ReferenceSkeletonEncoder(
            hidden_dim=dim,
            edge_hidden_dim=reference_encoder_edge_dim,
            num_blocks=reference_encoder_num_blocks,
            num_attention_heads=num_attention_heads,
            intermediate_size=intermediate_size,
            grid_size=grid_size,
    ...
    def init_weights(self, buffer_device=None):
        ...
        self.reference_skeleton_encoder.init_rope()
        ...
        if pretrained_path is not None:
            ...
            # The reference encoder and joint type are the only additions to NEXUS.
            current_state = self.state_dict()
            for key, value in current_state.items():
                if key.startswith(("reference_skeleton_encoder.", "joint_type_embed.")):
    ...
    def forward(
        ...
        reference_joint_positions: torch.Tensor = None,
        reference_joint_rope_positions: torch.Tensor = None,
        reference_edge_index: torch.Tensor = None,
        reference_cu_seqlens: torch.Tensor = None,
    ) -> torch.Tensor:
        """
        前向传播 - Varlen 模式
        ...
        if joint_mask is None:
            raise ValueError("Reference-skeleton training requires a joint mask")
        reference_inputs = (
            reference_joint_positions,
            reference_joint_rope_positions,
            reference_edge_index,
            reference_cu_seqlens,
        )
        if any(value is None for value in reference_inputs):
            raise ValueError("Reference-skeleton training requires positions, edges, and cu_seqlens")
        if int(joint_mask.sum()) != len(reference_joint_positions):
            raise ValueError("Reference features and joint token count do not match")
        semantic = self.reference_skeleton_encoder(
            positions=reference_joint_positions,
            rope_positions=reference_joint_rope_positions,
            edge_index=reference_edge_index,
            cu_seqlens=reference_cu_seqlens,
        )
        token_type = self.joint_type_embed.weight[0].view(1, -1)
        x[joint_mask] = x[joint_mask] + (semantic + token_type).to(x.dtype)
    ...
    def train_step(
        ...
        pred = self(
            ...
            reference_joint_positions=input_dict.reference_joint_positions_flat,
            reference_joint_rope_positions=input_dict.reference_joint_rope_positions_flat,
            reference_edge_index=input_dict.reference_edge_index_flat,
            reference_cu_seqlens=input_dict.reference_cu_seqlens,
        )
