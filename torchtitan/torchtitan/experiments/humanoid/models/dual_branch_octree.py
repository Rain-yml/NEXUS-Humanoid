"""Frozen NEXUS mesh branch with a trainable semantic-joint branch."""
...
class JointAttendMesh(nn.Module):
    """Update joint tokens by cross-attending to detached mesh tokens."""
    ...
    def __init__(
        ...
        super().__init__()
        self.norm = FP32LayerNorm(hidden_size, elementwise_affine=False)
    ...
    def forward(
        ...
        mesh_hidden_states: torch.Tensor,
        joint_cu_seqlens: torch.Tensor,
        mesh_cu_seqlens: torch.Tensor,
        max_joint_seqlen: int,
        max_mesh_seqlen: int,
    ) -> torch.Tensor:
        dtype = joint_hidden_states.dtype
        update = self.attention(
            hidden_states=self.norm(joint_hidden_states).to(dtype),
            encoder_hidden_states=mesh_hidden_states.detach(),
            cu_seqlens_q=joint_cu_seqlens,
            cu_seqlens_kv=mesh_cu_seqlens,
            max_seqlen_q=max_joint_seqlen,
            max_seqlen_kv=max_mesh_seqlen,
        )
        return joint_hidden_states + update
...
class JointOctreeDiffusionModel(ModelMixin, ConfigMixin):
    """
    八叉树逐层扩散模型
    ...
    def forward(
        ...
        attend_contexts: Optional[List[torch.Tensor]] = None,
        attend_context_cu_seqlens: Optional[torch.Tensor] = None,
        attend_modules: Optional[nn.ModuleList] = None,
        ...
        max_context_seqlen = (
            torch.diff(attend_context_cu_seqlens).max().item()
            if attend_context_cu_seqlens is not None
            else None
        )
        if attend_contexts is not None:
            if attend_modules is None or attend_context_cu_seqlens is None:
                raise ValueError("Joint attention requires modules and mesh sequence boundaries")
            if len(attend_contexts) != len(self.layers) or len(attend_modules) != len(self.layers):
                raise ValueError("Joint attention must provide one mesh context and adapter per layer")
        ...
        for layer_index, block in enumerate(self.layers):
            ...
            if attend_contexts is not None:
                x = attend_modules[layer_index](
                    joint_hidden_states=x,
                    mesh_hidden_states=attend_contexts[layer_index],
                    joint_cu_seqlens=cu_seqlens_q,
                    mesh_cu_seqlens=attend_context_cu_seqlens,
                    max_joint_seqlen=max_seqlen_q,
                    max_mesh_seqlen=max_context_seqlen,
                )
        ...
        if return_hidden_states:
            return x, hidden_states_by_layer
        return x
...
class DualBranchOctreeDiffusionModel(nn.Module):
    """Two NEXUS octree branches with one-way joint-attend-mesh conditioning."""

    _supports_gradient_checkpointing = True
    _no_split_modules = ["OctreeDiTBlock", "JointAttendMesh"]

    def __init__(self, **branch_kwargs) -> None:
        super().__init__()
        self.mesh_branch = JointOctreeDiffusionModel(**branch_kwargs)
        self.joint_branch = JointOctreeDiffusionModel(**branch_kwargs)
        self.joint_attend_mesh = nn.ModuleList(
            [
                JointAttendMesh(
                    layer_idx=layer_idx,
                    hidden_size=branch_kwargs["dim"],
                    num_attention_heads=branch_kwargs["num_attention_heads"],
                    num_key_value_heads=branch_kwargs.get("num_key_value_heads"),
                    qk_norm=branch_kwargs["qk_norm"],
                    qk_norm_eps=branch_kwargs["qk_norm_eps"],
                    attention_bias=branch_kwargs["attention_bias"],
                    use_flash_attn_3=branch_kwargs["use_flash_attn_3"],
                )
                for layer_idx in range(branch_kwargs["num_layers"])
            ]
        )
        self.freeze_mesh_branch()
    ...
    def init_weights(self, buffer_device=None) -> None:
        self.mesh_branch.init_weights(buffer_device=buffer_device)
        self.joint_branch.init_weights(buffer_device=buffer_device)
        for adapter in self.joint_attend_mesh:
            nn.init.zeros_(adapter.attention.o_proj.weight)
            if adapter.attention.o_proj.bias is not None:
                nn.init.zeros_(adapter.attention.o_proj.bias)
        self.freeze_mesh_branch()

    def is_scalar_param(self, name, param) -> bool:
        if name.startswith("mesh_branch."):
            return self.mesh_branch.is_scalar_param(name.removeprefix("mesh_branch."), param)
        if name.startswith("joint_branch."):
            return self.joint_branch.is_scalar_param(name.removeprefix("joint_branch."), param)
        return param.ndim <= 1

    def forward(
        self,
        *,
        mesh_x_t: torch.Tensor,
        joint_x_t: torch.Tensor,
        mesh_t: torch.Tensor,
        joint_t: torch.Tensor,
        mesh_centers: torch.Tensor,
        joint_centers: torch.Tensor,
        mesh_depths: torch.Tensor,
        joint_depths: torch.Tensor,
        mesh_cu_seqlens: torch.Tensor,
        joint_cu_seqlens: torch.Tensor,
        joint_ids: torch.Tensor,
        num_layers_per_mesh: list,
        encoder_hidden_states: torch.Tensor,
        condition_cu_seqlens: torch.Tensor = None,
        num_vertices: torch.Tensor = None,
        quad_ratios: torch.Tensor = None,
        symmetries: torch.Tensor = None,
        view_indices: torch.Tensor = None,
        mv_cu_seqlens: torch.Tensor = None,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        mesh_prediction, mesh_hidden_states = self.mesh_branch(
            x_t=mesh_x_t,
            t=mesh_t,
            centers=mesh_centers,
            depths=mesh_depths,
            cu_seqlens_q=mesh_cu_seqlens,
            num_layers_per_mesh=num_layers_per_mesh,
            encoder_hidden_states=encoder_hidden_states,
            condition_cu_seqlens=condition_cu_seqlens,
        ...
        joint_prediction = self.joint_branch(
            x_t=joint_x_t,
            t=joint_t,
            ...
            mv_cu_seqlens=mv_cu_seqlens,
            joint_ids=joint_ids,
            joint_mask=torch.ones_like(joint_ids, dtype=torch.bool),
            attend_contexts=mesh_hidden_states,
            attend_context_cu_seqlens=mesh_cu_seqlens,
            attend_modules=self.joint_attend_mesh,
        )
        return mesh_prediction, joint_prediction

    def flops(self, mesh_cu_seqlens=None, condition_lens=None, condition_proj_tokens=None):
        return self.mesh_branch.flops(
            mesh_cu_seqlens, condition_lens, condition_proj_tokens
        )
