# Owned copy of experiments/vem/pipelines/i2v.py.
...
@dataclass
class ImageToJointOctreePipelineOutput(BaseOutput):
    vertices: torch.Tensor
    vertices_layers: List[torch.Tensor]
    joints: torch.Tensor
    joint_layers: List[torch.Tensor]

class ImageToJointOctreePipeline(DiffusionPipeline):
    def __init__(
        self,
    ...
    def sample_layer(
        ...
        all_centers = torch.cat([centers, joint_centers], dim=0)
        token_joint_ids = torch.cat(
            [torch.full((n_mesh_tokens,), -1, device=device, dtype=torch.long), joint_ids]
        )
        joint_mask = token_joint_ids >= 0
        n_tokens = all_centers.shape[0]
        ...
        sub_voxel_centers = all_centers.view(n_tokens, 1, 3) + child_offsets * sub_voxel_size // 2
        mesh_values = x_t[:n_mesh_tokens]
        sub_centers = sub_voxel_centers[:n_mesh_tokens][mesh_values.float() > 0]
        joint_values = x_t[n_mesh_tokens:]
        joint_children = joint_values.argmax(dim=-1)
        next_joint_centers = sub_voxel_centers[n_mesh_tokens:][
    ...
    def __call__(
        ...
        return ImageToJointOctreePipelineOutput(
            vertices=centers,
