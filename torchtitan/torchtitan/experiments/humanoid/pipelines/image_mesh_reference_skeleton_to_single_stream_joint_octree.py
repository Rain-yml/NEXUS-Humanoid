"""Reference-skeleton joint rollout with clean ground-truth mesh layers."""
...
@dataclass
class ImageMeshReferenceSkeletonToSingleStreamJointOctreePipelineOutput(BaseOutput):
    joints: torch.Tensor
    joint_layers: List[torch.Tensor]
    joint_ids: torch.Tensor


class ImageMeshReferenceSkeletonToSingleStreamJointOctreePipeline(DiffusionPipeline):
    """Denoise joints from an encoded reference skeleton and clean mesh tokens."""
    ...
    def sample_layer(
        ...
        *,
        ...
        reference_joint_rope_positions: torch.Tensor,
        reference_edge_index: torch.Tensor,
        reference_cu_seqlens: torch.Tensor,
    ) -> torch.Tensor:
        ...
        def predict(condition: torch.Tensor, timestep: torch.Tensor):
            ...
            prediction = self.octree_dit(
                ...
                reference_joint_rope_positions=reference_joint_rope_positions,
    ...
    def __call__(
        ...
        *,
        ...
        reference_joint_rope_positions: torch.Tensor,
        reference_edge_index: torch.Tensor,
    ) -> ImageMeshReferenceSkeletonToSingleStreamJointOctreePipelineOutput:
        if not mesh_layers:
        ...
        reference_joint_positions = torch.as_tensor(
            reference_joint_positions, dtype=torch.float32, device=device
        )
        reference_joint_rope_positions = torch.as_tensor(
            reference_joint_rope_positions, dtype=torch.long, device=device
        )
        reference_edge_index = torch.as_tensor(
            reference_edge_index, dtype=torch.long, device=device
        )
        num_joints = joint_ids.numel()
        if reference_joint_positions.shape != (num_joints, 3):
            raise ValueError(
                "reference_joint_positions must match the requested joint tokens"
            )
        if reference_joint_rope_positions.shape != (num_joints, 3):
            raise ValueError(
                "reference_joint_rope_positions must match the requested joint tokens"
            )
        if reference_edge_index.ndim != 2 or reference_edge_index.shape[0] != 2:
            raise ValueError("reference_edge_index must be shaped (2, E)")
        if reference_edge_index.numel() and (
            reference_edge_index.min() < 0
            or reference_edge_index.max() >= num_joints
        ):
            raise ValueError("reference_edge_index contains an out-of-range joint")
        reference_cu_seqlens = torch.tensor(
            [0, num_joints], dtype=torch.int32, device=device
        )
        ...
        return ImageMeshReferenceSkeletonToSingleStreamJointOctreePipelineOutput(
