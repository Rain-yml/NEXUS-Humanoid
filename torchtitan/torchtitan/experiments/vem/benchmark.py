...
def mock_data(bs, sequence_length, device):
    cond_pc = torch.randn(bs, 153600, 6, device=device)
    cond_salient_pc = torch.randn(bs, 8192, 3, device=device)
    input_ids = torch.nested.nested_tensor([torch.randint(0, 100, (sequence_length,), device=device) for _ in range(bs)], dtype=torch.long, layout=torch.jagged)
    position_ids = torch.nested.nested_tensor([torch.arange(sequence_length, device=device) for _ in range(bs)], dtype=torch.long, layout=torch.jagged)
    seq_mask = torch.ones(bs * sequence_length, dtype=torch.bool, device=device)
    return {
        'cond_pc': cond_pc,
        'cond_salient_pc': cond_salient_pc,
        'input_ids': input_ids,
...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        # build dataloader
        tokenizer = self.train_spec.build_tokenizer_fn(job_config.tokenizer)
        # 
        ...
        model_args.adapt_to_tokenizer(tokenizer, world_size=job_config.training.train_world_size or dp_degree)
    ...
    def forward_model(self, model, input_dict):
        pred = model(
            ...
            noise_interpolate=self.job_config.training.noise_interpolate,
            point_cloud_salient=input_dict.get("cond_salient_pc", None),
        )
    ...
    def train_step(self, input_dict: dict[str, torch.Tensor]):
        ...
        with self.train_context(optional_context_parallel_ctx):
            ...
            loss, log_dict, metrics = self.forward_model(model, input_dict)
            # TODO
    ...
    def train(self):
        ...
        seqs = [100000,30000,25000,20000,10000,5000,3000,2000,500,200]
        time_costs = []
        bs = 1
        model = self.model_parts[0]
        for seq in seqs:
            start = time.time()
            for i in range(100):
                input_dict = mock_data(bs, seq, self.device)
                self.optimizers.zero_grad()
            ...
            print("total time", seq, end - start)
            time_costs.append(end - start)

        plt.plot(seqs, time_costs)
        dst_path = os.path.join(job_config.job.dump_folder, 'time_costs.png')
