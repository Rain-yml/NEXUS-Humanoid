...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def forward_model(self, model, input_dict):
        loss, log_dict = model.train_step(
            input_dict,
        )
        return loss, {
        ...
        if (
            ...
            for key, value in metrics.items():
                if isinstance(value, torch.Tensor):
                    metrics[key] = dist_utils.dist_mean(value.detach(), world_mesh["dp_cp"], ft_pg)
                else:
                    metrics[key] = dist_utils.dist_mean(torch.tensor(value, device=self.device), world_mesh["dp_cp"], ft_pg)
        else:
    ...
    def validate(
        ...
        for input_dict in self.val_dataloader:
            for k in input_dict:
                if isinstance(input_dict[k], torch.Tensor):
                    input_dict[k] = input_dict[k].to(device_type)
            ...
            with self.train_context(optional_context_parallel_ctx):
                assert len(model_parts) == 1
                loss, metrics = self.forward_model(model, input_dict)
                print(f"Validation step {num_steps}, loss {loss.item():.4f}, acc {metrics.get('accuracy', 0):.4f}")

            accumulated_losses.append(loss.detach())
            accumulated_acces.append(metrics.get("accuracy", 0))
            num_steps += 1
        ...
        # Compute average acc
        acc = torch.sum(torch.stack(accumulated_acces))
        acc /= num_steps
        ...
        if (
            ...
            ft_pg = self.ft_manager.replicate_pg if self.ft_manager.enabled else None
            global_avg_loss = dist_utils.dist_mean(loss, world_mesh["dp_cp"], ft_pg)
        else:
            global_avg_loss = loss.item()

        self.metrics_processor.log_validation(loss=global_avg_loss, step=step, extra_metrics={"acc": acc})
        ...
        with maybe_enable_profiling(
            ...
            while self.step < job_config.training.steps:
                ...
                if (
                    ...
                    and (
                        self.step == 1 or self.step % job_config.validation.interval == 0
                    )
                ):
                    self.validate(self.model_parts, self.step)
