...
class Trainer(torch.distributed.checkpoint.stateful.Stateful):
    ...
    def __init__(self, job_config: JobConfig):
        ...
        # build dataloader
        self.tokenizer = tokenizer = self.train_spec.build_tokenizer_fn(job_config.tokenizer)
        # 
        ...
        model_args.adapt_to_tokenizer(tokenizer, world_size=job_config.training.train_world_size)
    ...
    def forward_model(self, model, input_dict):
        pred = model(
            ...
            point_cloud=input_dict["cond_pc"],
            position_ids=input_dict["position_ids"],
        )
        loss = model.loss_fn(pred, input_dict)
        return loss, {
            "num_tokens": pred["num_tokens"],
            "num_flops": pred["num_flops"],
        }
    ...
    def train_step(self, input_dict: dict[str, torch.Tensor]):
        ...
        with self.train_context(optional_context_parallel_ctx):
            ...
            loss, metrics = self.forward_model(model, input_dict)
            # TODO
        ...
        self.metrics_processor.log(self.step, global_avg_loss, global_max_loss, extra_metrics={"grad_norm": grad_norm.item()})
    ...
    def val(self):
        ...
        if not loaded:
            logger.error("No checkpoint is loaded, val starts from scratch.")
            return
        ...
        with maybe_enable_profiling(
            ...
            for input_dict in self.dataloader:
                device_type = utils.device_type
                for k, _ in input_dict.items():
                    if isinstance(input_dict[k], torch.Tensor):
                        input_dict[k] = input_dict[k].to(device_type)
                self.inference(input_dict)
    ...
    def inference(self, input_dict):
        ...
        bs = input_dict['cond_pc'].shape[0]
        n = self.job_config.training.sampling_params.n
        out_dir = os.path.join(config.job.dump_folder, 'val')
        rank = torch.distributed.get_rank()
        os.makedirs(out_dir, exist_ok=True)

        for i in range(bs):
            pc = input_dict['cond_pc'][i].cpu().numpy()
            pc_ply = trimesh.PointCloud(vertices=pc[:, :3], colors=((pc[:, 3:] * 0.5 + 0.5) * 255).astype(np.uint8))
            rot_x_cw = trimesh.transformations.rotation_matrix(-np.pi / 2, [1, 0, 0])
            pc_ply.apply_transform(rot_x_cw)
            pc_ply.export(os.path.join(out_dir, f"step{self.step}-rank{rank:02d}_{i:02d}_cond.ply"))
            
        result = model.inference(input_dict, sampling_params=self.job_config.training.sampling_params)

        for i in range(bs):
            for j in range(n):
                tokens = result['sequences'][i * n + j].detach().cpu().numpy()
                v, f = self.tokenizer.detokenize(tokens)
                mesh = trimesh.Trimesh(vertices=v, faces=f)
                mesh.apply_transform(rot_x_cw)
                mesh.export(os.path.join(out_dir, f"step{self.step}-rank{rank:02d}_{i:02d}_sample{j:02d}.obj"))
