...
from dataclasses import asdict, dataclass, field, fields, is_dataclass, make_dataclass, MISSING
from typing import Any, Literal, Type, List
...
class Optimizer:
    ...
    """Optimizer to use for DeepSeek MoE routing layers when using Dion."""

    expert_optimizer: Literal["adamw", "lion"] = "adamw"
    """Optimizer to use for MoE expert weights when using Dion."""
...
class Training:
    ...
    mixed_precision_param: Literal["bfloat16", "float32", "float16"] = "bfloat16"
    """
...
class Checkpoint:
    ...
    load_path_override: str | None = None
    load_model_exclude_keys: list[str] = field(default_factory=list)
    ...
    persistent_steps: list[int] = field(default_factory=list)
    """
    A list of steps at which to save a persistent checkpoint.
    """
@dataclass
...
class ConfigManager:
    ...
    @staticmethod
    def _field_to_make_dataclass_arg(dfield):
        """
        Convert a dataclasses.Field (from dataclasses.fields(...)) into
        an appropriate dataclasses.field(...) argument for make_dataclass.
        Returns a `dataclasses.Field` produced by dataclasses.field(...) or None
        if there is no default/default_factory.
        """
        # dfield is a dataclasses.Field (from dataclasses.fields)
        if dfield.default is not MISSING:
            return field(default=dfield.default)
        if getattr(dfield, "default_factory", MISSING) is not MISSING:
            # Field.default_factory exists on dataclasses.Field
            return field(default_factory=dfield.default_factory)
        return None
    ...
    def _merge_configs(base, custom) -> Type:
        ...
        - If a field exists in both `base` and `custom`:
            - If both field types are dataclasses, they are merged recursively.
            - Otherwise, the field from `custom` overrides the one in `base`.
        - Fields only present in `base` or `custom` are preserved as-is,
          keeping their default or default_factory if present.
        """
        result = []
        b_map = {f.name: f for f in fields(base)}
        c_map = {f.name: f for f in fields(custom)}

        for name, bf in b_map.items():
            if (
                name in c_map
                and is_dataclass(bf.type)
                and is_dataclass(c_map[name].type)
            ):
                # both are dataclasses: merge recursively and use factory for merged type
                m_type = ConfigManager._merge_configs(bf.type, c_map[name].type)
                result.append((name, m_type, field(default_factory=m_type)))

            elif name in c_map:
                # custom overrides base: preserve custom default/default_factory if present
                cf = c_map[name]
                custom_default = ConfigManager._field_to_make_dataclass_arg(cf)
                if custom_default is not None:
                    result.append((name, cf.type, custom_default))
                else:
                    # no explicit default on custom: declare field without default
                    result.append((name, cf.type))

            else:
                # only in base: preserve base default/default_factory if present
                base_default = ConfigManager._field_to_make_dataclass_arg(bf)
                if base_default is not None:
                    result.append((name, bf.type, base_default))
                else:
                    result.append((name, bf.type))

        # fields only in custom
        for name, cf in c_map.items():
            if name not in b_map:
                custom_default = ConfigManager._field_to_make_dataclass_arg(cf)
                if custom_default is not None:
                    result.append((name, cf.type, custom_default))
                elif is_dataclass(cf.type):
                    # If it's a dataclass type with no explicit default, create by default_factory
                    result.append((name, cf.type, field(default_factory=cf.type)))
                else:
                    # primitive or no default — declare without default
                    result.append((name, cf.type))
